...
Ejecuta: pytest 2-fase2/test_deberta.py -v
"""
...
_MODEL_DIR = os.path.join(
    _PROJECT_ROOT, "3-LLM-judge", "modelsBert", "guard"
)
...
class TestModelLoading:
    ...
    def test_model_is_deberta(self, model_and_tokenizer):
        model, _, _ = model_and_tokenizer
        model_type = getattr(model.config, "model_type", "")
        assert "deberta" in model_type.lower(), f"Expected DeBERTa, got: {model_type}"
...
class TestBasicPredictions:
    ...
    @pytest.mark.parametrize("prompt,expected_label", [
        ...
        ("Summarize the article about climate change", "benign"),
    ])
...
class TestDatasetMetrics:
    ...
    def test_metrics(self, model_and_tokenizer, test_data):
        ...
        assert f1 > 0.90, f"F1 too low: {f1:.4f}"
...
class TestLatency:
    ...
    def test_latency_single_prompt(self, model_and_tokenizer):
        ...
        assert avg_ms < 1000, f"Average latency too high: {avg_ms:.2f} ms"
    ...
    def test_latency_batch(self, model_and_tokenizer, test_data):
        ...
        assert total_ms < 120000, f"Batch too slow: {total_ms:.0f} ms"
